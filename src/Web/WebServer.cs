...
namespace TemperatureWarriorCode.Web
{
    ...
    // NOTA: Este código expande en la implementación de
    // https://developer.mozilla.org/en-US/docs/Web/API/WebSockets_API/Writing_WebSocket_server
    // NOTA: No se usa el soporte a WebSocket que brinda C# en el módulo Http porque
    // al parecer no está soportado por netstandard2.1 (o por el subconjunto que
    // posiblemente use Meadow). Usar ese módulo puede compilar pero las peticiones
    // nunca son detectadas como WebSocket (Request.IsWebSocketRequest siempre es
    // falso).
    class WebSocketServer
    {
        ...
        protected void OnMessageReceived(NetworkStream connection, Message message) =>
            MessageReceived.Invoke(this, connection, message);

        public delegate void ConnectionFinishedHandler(
            WebSocketServer webServer,
            NetworkStream connection
        );
        public event ConnectionFinishedHandler ConnectionFinished = delegate { };

        protected void OnConnectionFinished(NetworkStream connection) =>
            ConnectionFinished.Invoke(this, connection);
        ...
        private async Task HandleConnection(TcpClient client)
        {
            ...
            // Decidí no hacer una variable miembro del WebsocketServer que la
            // almacene para facilitar el manejo concurrente de más de una conexión.
            NetworkStream connection = client.GetStream();
            byte[] packet = new byte[512];
            int readBytes = 0;
            while (readBytes < 3)
            { // esperar por "GET" para iniciar handshake
                var rb = connection.Read(
                    new ArraySegment<byte>(packet, readBytes, packet.Length - readBytes)
                );
                if (rb <= 0)
                {
            ...
            {
                Resolver.Log.Error("[WebServer] Handshake WebSocket inválido. GET no encontrado");
                return;
            }
            ...
            while (true)
            {
                ...
                // El header más pequeño de un paquete es de 2 bytes (2 bytes header
                // básico + 0 payload)
                ...
                // Tenemos un mensaje como la gente (a no ser que recibamos algo que
                // no sea reconocido como mensaje websocket, en tal caso explota el
                // mundo)
                Message message;
                if (!Message.TryParse(payload, out message))
                {
                    ...
                    continue; // Mensaje en formato inválido. Ignorar (quizás
                    // deberíamos fallar)
                }
        ...
        {
            // 1. Obtain the value of the "Sec-WebSocket-Key" request header without
            // any leading or trailing whitespace
            // 2. Concatenate it with "258EAFA5-E914-47DA-95CA-C5AB0DC85B11" (a
            // special GUID specified by RFC 6455)
            // 3. Compute SHA-1 and Base64 hash of the new value
            // 4. Write the hash back as the value of "Sec-WebSocket-Accept"
            // response header in an HTTP response
            string swk = Regex
                .Match(stringifiedPacket, "Sec-WebSocket-Key: (.*)")
                .Groups[1]
                .Value.Trim();
            string swka = swk + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
            byte[] swkaSha1 = System
        ...
        private string? PacketToPayload(byte[] packet)
        {
            ...
            // must be true, "All messages from the client to the server have this
            // bit set"
            bool mask = (packet[1] & 0b10000000) != 0;
            ...
            // - No manejamos chunks de texto en distintos paquetes por lo que fin
            // debe estar seteado.
            // - Mask siempre debe estar seteado en mensajes del cliente.
            // if (opcode != 1 || !fin || !mask)
            //    return null;

            // offset mínimo del mensaje (según msglen puede ser mayor, ver switch
            // debajo)
            ulong offset = 2;
            ...
            switch (msglen)
            {
                case 126:
                {
                    ...
                    // bytes tomados de mayor índice en packet a menor índice en packet
                    // porque vienen en orden de red (Big-Endian) y el BitConverter los
                    // espera en Little-Endian - porque aparentemente el Cortex M7 de la
                    // Meadow está configurado para little-endian
